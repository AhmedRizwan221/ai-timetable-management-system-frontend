...
export default function ManageDepartments() {
    ...
    // console.log(user);
    const { departments = [], error, totalPages, currentPage, loading, hasPrevPage, hasNextPage } = useSelector((state) => state.department);
    ...
    const filterDepartments = departments.filter((dept) =>
        dept?.deptName.toLowerCase().includes(search.trim().toLocaleLowerCase()) ||
        dept?.chairman?.fullName?.toLowerCase().includes(search.trim().toLowerCase())
    )
    ...
    return (
        <div className="min-h-screen bg-muted/30 md:py-10 md:px-4">
            <div className="bg-white border border-gray-200 rounded-lg">
                ...
                {/* mobile screen */}
                <div className="grid grid-cols-1 gap-4 md:hidden px-2 md:p-0">
                    {loading ? (
                        <div>
