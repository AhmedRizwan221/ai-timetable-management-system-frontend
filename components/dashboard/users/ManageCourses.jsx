...
import { Pencil, Trash, Book, Search, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ManageCourses() {
    ...
    const filterCourses = courses.filter((fact) =>
        fact?.courseName?.toLowerCase().includes(search.trim().toLowerCase()) ||
        ...
        <div className="min-h-screen bg-muted/30 md:py-10 md:px-4">
            <div className="bg-white border border-gray-200 rounded-lg">
                <div className="bg-card">
                    <SearchInput
                        heading={"Courses Managment"}
                        icon={<Search />}
                        search={search}
                        setSearch={setSearch}
                        HeadingIcon={<Book className="h-10 w-10 text-primary-foreground" />}
